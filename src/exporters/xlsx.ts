...
const COLORS = ["B8E0D2","FFD6A5","BDE0FE","E4C1F9","F7CAD0","CDEAC0","FFF1A8"];
...
export async function buildWorkbook(result: AdvisorResult): Promise<Buffer> {
  ...
  summary.addRow(["策略", "总学分", "课程数", "培养要求覆盖", "警告"]);
  result.strategies.forEach((plan) => summary.addRow([LABELS[plan.strategy], plan.totalCredits, plan.sections.length, plan.requirementCoverage.join("；"), plan.warnings.join("；")]));
  styleHeader(summary, 5);
  summary.columns = [{width:16},{width:12},{width:12},{width:48},{width:48}];
...
function addPlanSheet(workbook: ExcelJS.Workbook, plan: RecommendedPlan): void {
  ...
  for(let row=3;row<=15;row++){sheet.getRow(row).height=52;for(let col=2;col<=8;col++){const cell=sheet.getCell(row,col);cell.alignment={wrapText:true,vertical:"middle",horizontal:"center"};if(cell.value){const code=String(cell.value).split(" ")[0];const index=Math.max(0,plan.sections.findIndex(section=>section.code===code));cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:COLORS[index%COLORS.length]}};}}}
