...
run_npm view "sustech-course-advisor@$ADVISOR_VERSION" version --json >/dev/null || { echo "sustech-course-advisor@$ADVISOR_VERSION is not available from the selected npm registry." >&2; exit 1; }
run_npm view "sustech-cli@$SUSTECH_VERSION" version --json >/dev/null || { echo "sustech-cli@$SUSTECH_VERSION is not available from the selected npm registry." >&2; exit 1; }
run_npm install --prefix "$PACKAGE_ROOT" --omit=dev --no-audit --no-fund "sustech-course-advisor@$ADVISOR_VERSION" "sustech-cli@$SUSTECH_VERSION"
