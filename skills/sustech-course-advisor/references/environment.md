...
The scripts first verify that both exact package versions exist in the selected npm registry, then install them into an isolated per-user directory. They do not use `sudo`, modify the system Node installation, edit shell startup files, or change the global `PATH`. If the existing Node.js is older than 20.18.0, they download the official Node.js archive from `nodejs.org`, verify it against the release `SHASUMS256.txt`, and use that isolated runtime. After installation they run `doctor` against the newly installed CLI and require `installationReady: true`; authentication may remain independently not ready. They stop on unsupported platforms, package unavailability, missing checksum tooling, verification failure, package-install failure, or capability-contract failure.
